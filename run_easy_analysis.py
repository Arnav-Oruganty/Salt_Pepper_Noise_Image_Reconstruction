...
RESULTS_DIR = "easy_version_results_small"
NOISE_LEVELS = [5, 8, 10]
LAMBDA_VALUES = [0.1, 1, 1.2, 1.4, 5]
NUM_SAMPLES_TO_SAVE = 3
NUM_IMAGES_TO_PROCESS = 5 # Use only 5 images
...
# Select the first 5 images from the directory to keep them fixed across runs
image_files = sorted(os.listdir(ground_truth_dir))[:NUM_IMAGES_TO_PROCESS]

print(f"Processing {len(image_files)} fixed images.")
...
for noise_level in NOISE_LEVELS:
    ...
    for i, filename in enumerate(image_files):
        ...
        for lam in LAMBDA_VALUES:
            X_recon = solve_easy_problem(X_corr, lam)
...
for noise_level in NOISE_LEVELS:
    ...
    plt.figure()
    plt.plot(lambdas, avg_mses, marker='o')
    plt.xscale('log')
    plt.xlabel('Lambda (λ)')
    plt.ylabel('Average Mean Squared Error (MSE)')
    plt.title(f'Easy Problem: MSE vs. Lambda for {noise_level}% Noise')
